...
class PrpAssignmentScreen extends StatefulWidget {
  const PrpAssignmentScreen(
      {required this.session, super.key, this.locationService});
...
class _PrpAssignmentScreenState extends State<PrpAssignmentScreen> {
  late final DeviceLocationService locations =
  ...
  Future<void> locate() async {
    ...
    try {
      final point = await locations.determineCurrentPosition();
  ...
  Future<void> transition(String action, String successMessage) async {
    ...
    if (success) {
      ...
      if (action == 'complete') {
        await widget.session.refreshAssignment(silent: true);
  ...
  Future<void> openNavigation(GeoPoint target) async {
    ...
    final uri = Uri(
        scheme: 'geo',
        path: coordinates,
        queryParameters: {'q': '$coordinates(SafeZone PRP)'});
    if (!await launchUrl(uri, mode: LaunchMode.externalApplication) &&
        mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content:
              Text('No navigation application is available on this device.')));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (assignment == null) {
      return const Scaffold(
          body: Center(child: Text('This assignment is no longer active.')));
    }
    final target = assignment.prpLocation;
    final distance = currentLocation == null
        ? null
        : locations.distanceMeters(currentLocation!, target);
    final markers = [
      Marker(
          ...
          child: const Icon(Icons.location_pin,
              size: 46, color: Color(0xFFB42318))),
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('PRP assignment'), actions: [
        IconButton(
                  ...
                  if (currentLocation case final point?)
                    PolylineLayer(polylines: [
                      Polyline(
                          ...
                          color: SafeZoneTheme.blue.withValues(alpha: .65),
                          strokeWidth: 4,
            ...
            child: Column(
                ...
                children: [
                  Row(
                      ...
                      children: [
                        Expanded(
                            child: Text('Assigned patrol point',
                                style: Theme.of(context)
                                    .textTheme
                                    .titleLarge
                                    ?.copyWith(fontWeight: FontWeight.w800))),
                        _Badge(assignment.status)
                      ]),
                      ...
                      child: Padding(
                          ...
                          child: Column(children: [
                            _Info(
                                ...
                                label: 'Current shift',
                                value:
                                    '${formatDateTime(assignment.shiftStart)}\n${formatDateTime(assignment.shiftEnd)}'),
                            const Divider(height: 25),
                            _Info(
                                icon: Icons.route_outlined,
                                label: 'Straight-line proximity',
                                value: locating
                                    ? 'Locating…'
                                    : distance == null
                            ...
                            const _Info(
                                ...
                                label: 'Road route / ETA',
                                value:
                                    'Unavailable from the current mobile API'),
                          ]))),
                  if (locationError case final message?) ...[
                    const SizedBox(height: 13),
                    ErrorNotice(message),
                    TextButton.icon(
                        onPressed: locations.openSettings,
                  ...
                  OutlinedButton.icon(
                      onPressed: () => openNavigation(target),
                      icon: const Icon(Icons.navigation_outlined),
                      label: const Text('Open in navigation app')),
                  const SizedBox(height: 10),
                  if (assignment.canAcknowledge)
                    FilledButton.icon(
                        onPressed: widget.session.refreshing
                            ? null
                            : () => transition(
                                'acknowledge', 'Assignment acknowledged.'),
                        icon: const Icon(Icons.check_circle_outline),
                        label: const Text('Acknowledge assignment')),
                  if (assignment.canArrive)
                    FilledButton.icon(
                        onPressed: widget.session.refreshing
                            ? null
                            : () => transition(
                                'arrive', 'Arrival at PRP recorded.'),
                        icon: const Icon(Icons.location_on_outlined),
                        label: const Text('Mark arrived at PRP')),
                  if (assignment.canComplete) ...[
                    ...
                    OutlinedButton.icon(
                        onPressed: widget.session.refreshing
                            ? null
                            : () =>
                                transition('complete', 'Assignment completed.'),
                        icon: const Icon(Icons.flag_outlined),
                        label: const Text('Complete assignment'))
                  ],
                  const SizedBox(height: 14),
                  Text(
                      'Assignment ${shortId(assignment.id)} · Patrol ${shortId(assignment.patrolUnitId)}',
...
class _Info extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Icon(icon, color: SafeZoneTheme.blue),
